...
async def startup_event():
    ...
    try:
        ...
        # Create demo data for admin portal testing
...
async def create_demo_data():
    ...
    try:
        ...
        if user_count <= 1:  # Only admin exists
            ...
            demo_users = [
                ...
                },
                {
                    ...
                    "login_attempts": 3,
                    "locked_until": None,
                    "last_failed_login": datetime.utcnow() - timedelta(hours=6)  # This field was causing CSV issues
                }
            ]
        ...
        if ticket_count == 0:
            ...
            demo_tickets = [
                {
                    "ticket_id": f"ticket_{secrets.token_urlsafe(8)}",
            ...
            await db.support_tickets.insert_many(demo_tickets)
            print(f"✅ Created {len(demo_tickets)} demo support tickets")
        else:
            print(f"✅ Support tickets already exist ({ticket_count} total)")
            
        # Create demo email campaigns
        campaign_count = await db.email_campaigns.count_documents({})
        if campaign_count == 0:
            print("🔄 Creating demo email campaigns...")
            
            demo_campaigns = [
                {
                    "campaign_id": f"campaign_{secrets.token_urlsafe(8)}",
            ...
            await db.email_campaigns.insert_many(demo_campaigns)
            print(f"✅ Created {len(demo_campaigns)} demo email campaigns")
        else:
            print(f"✅ Email campaigns already exist ({campaign_count} total)")
