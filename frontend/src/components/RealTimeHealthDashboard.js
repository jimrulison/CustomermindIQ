...
const RealTimeHealthDashboard = ({ onNavigate }) => {
  ...
  const fetchHealthData = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/customer-health/dashboard`);
  ...
  const fetchAlerts = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/customer-health/alerts`);
  ...
  const resolveAlert = async (alertId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/customer-health/alerts/${alertId}/resolve`, {
        method: 'POST'
      });
  ...
  const setupWebSocket = () => {
    ...
    const wsUrl = `${process.env.REACT_APP_BACKEND_URL}/api/customer-health/ws/health-monitoring`.replace('http', 'ws');
